...
const doczRcBaseConfig = {
  ...
  filterComponents: files => files.filter(filepath => /\/[A-Z]\w*(\/index)?\.(js|jsx|ts|tsx)$/.test(filepath))
};
...
const execDevCommand = async ({
  ...
  spawn$1(gatsbyPath, ['develop', '--port', `${args.port}`], {
    stdio: 'inherit',
    cwd: docz
  });
...
const ensureFiles = ({
  ...
  const appPath = root;
  const themeDirs = glob.sync(path$1.join(args.themesDir, '/gatsby-theme-**'), {
    cwd: appPath,
  ...
  themeDirs.forEach(dir => {
    ...
    fs$1.copySync(dir, path$1.join(docz, 'src', themeName));
  });
